...
def test_0006_e():
    ...
    assert postag_left_check(pos_list, u"먹"), u"먹 in eojeol"
    assert postag_end_check(pos_list, u"었/EP+겠/EP+다/EF"), u"었/EP+겠/EP+다/EF in eojeol"

    pos_list = pos_E.endswithE(u"달리시겠어요.")
    assert postag_left_check(pos_list, u"달리"), u"달리 in eojeol"
    assert postag_end_check(pos_list, u"시/EP+겠/EP+어요/EF"), u"시/EP+겠/EP+어요/EF in eojeol"
...
def test_0007_e():
    ...
    # 규칙형 검사
    assert postag_left_check(pos_list, u"가"), u"가 in eojeol"
    assert postag_end_check(pos_list, u"시/EP+었/EP+다/EF"), u"시/EP+었/EP+다/EF in eojeol"
    # 불규칙형
