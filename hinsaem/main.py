...
class Hinsaem(object):
    ...
    def _readDict(self):
        ...
        with open(file_path0, 'r', encoding='UTF-8', newline='') as csvfile:
            next(csvfile, None) 
            for line in csv.reader(csvfile, delimiter="\t", dialect="excel-tab"):
                try:
                    word = line[0]
                    morpheme = line[1]
        ...
        config_dict = {"ALL" : multi_dict, "JOSA" : josa_set, "JOSA_LAST" : josa_last,
    ...
    def _parse_eojeol(self, eojeol):
        ...
        if eojeol in word_dict:
            union_set = set(word_dict[eojeol]) & set(self.SINGLE_EOJEOL_MORPHEME_LIST)
            if union_set != {}:
    ...
    def _check_irregular_emoi(self, eojeol, candidate_eomi):
        ...
        "르" 불규칙 : 어간 끝 음절 '르'가 받침 ㄹ로 줄고, 어미 '아/어','았/었/ 이 '라/러', '랐/렀' 으로 변하는 현상
    ...
    def check_irregular_eogan1(self, candidate_eogan, candidate_eomi):
        ...
        if parse_eumjeol(candidate_eogan[-1]) == "ㄹ" and parse_eumjeol(candidate_eomi[0])[0] == "o":
        ...
        #긋다=>그어, 그으니    낫다 => 나아, 나으니
        if parse_eumjeol(candidate_eogan[-1]) == "ㅅ" and parse_eumjeol(candidate_eomi[0])[0] == "o":
            jaso = parse_eumjeol(candidate_eogan[-1])
