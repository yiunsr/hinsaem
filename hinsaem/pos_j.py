...
class PosJ(PosBase):
    ...
    def endswithj(self, eojeol):
        ...
        for index in range(1, len(new_eojeol)+1):
            ...
            if leftword != ""  and josa != "":
                leftword_josa_list.append([index, leftword, josa, leftword[-1]])
            ...
            if jong in self._josa_jungjong_start:
                ...
                josa =  jong +josa
                leftword_josa_list.append([index, leftword, josa, last_eumjeol_left])
        ...
        for item in leftword_josa_list:
            ...
            new_candiate_list =self._get_candiate_info_list(index, eojeol, leftword, josa, last_eumjeol_left, mark, pos_filter)
    ...
    def _jungjong_only_josa(self, eojeol, mark):
        ...
        for posinfo in self._josa_jungjong_only[jong]:
            ...
            candiate_info = [leftword, postag_tuple, mark, {tuple(postag_tuple) : posinfo}]
            candiate_list.append(candiate_info)
        return candiate_list

    
    def _get_candiate_info_list(self, index, eojeol, candidate_leftword, candidate_josa, last_eumjeol_left, mark, pos_filter,):
        """
        ...
        if candidate_josa in self._josa_list:
            for posinfo in self._josa_list[candidate_josa]:
                if check_phoneme_restriction(last_eumjeol_left, posinfo["phoneme"]):
                    postag_tuple = self._pos_select(candidate_josa, posinfo["pos"], posinfo["pos2"])
                    # 추출하려는 형태소가 아니면 패스
                    if postag_tuple[-1][1] not in pos_filter:
                        continue
                    
                    candiate_info = [candidate_leftword, postag_tuple, mark, posinfo]
                    candiate_list.append(candiate_info)
